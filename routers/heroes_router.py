...
async def heartbeat(db: db_dependency):
    try:
        ...
        return {"error": str(e).encode("utf-8", errors="replace").decode("utf-8")}


# GET ALL (FOR THE LOGGED PLAYER)
@router.get("/", status_code=status.HTTP_200_OK)
async def get_all_heros(player: player_dependency, db: db_dependency):
    if player is None:
...
# GET BY RANK( AS QUERY PARAM) (FOR THE LOGGER PLAYER)
...
# GET ONE HERO BY NICKNAME (CAS PATH PARAM) (FOR THE LOGGER PLAYER)
...
# UPDATE WITH PUT (BY BODY) (FOR THE LOGGER PLAYER)
...
async def update_hero(player:player_dependency, db: db_dependency, hero_id: int = Path(ge=1), hero_body: HeroValidation = Body()):
    ...
    hero_db = db.query(Heroes).filter(Heroes.id == hero_id).filter(Heroes.owner_id == player.get("id")).first()
    if hero_db is None:
        raise HTTPException(status_code=404, detail="Herro can't be found")
...
# DELETE (BY ID AS PARAM) (FOR THE LOGGED PLAYER)
