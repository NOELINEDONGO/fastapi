...
def create_token(username: str, user_id: int,role:str, expires_delta: timedelta):
    ...
    return jwt.encode(encoded_data, settings.jwt_secret, algorithm=settings.jwt_algorithm)
...
async def login_player(form_data: Annotated[OAuth2PasswordRequestForm, Depends()],
                       db: db_dependency):
    player_authenticated = authenticate_player(form_data.username, form_data.password, db)
    if not player_authenticated:
        return "Wrong credentials"
    token = create_token(
        player_authenticated.username,
        player_authenticated.id,
        player_authenticated.role,
